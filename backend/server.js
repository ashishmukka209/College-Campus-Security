...
app.post("/getPasses", async (req, res) => {
  try {
    ...
    for (let index = 0; index < students.length; index++) {
      ...
      passes.forEach((pass) => {
        arr_passes.push({ rollno: pass.rollno, reason: pass.reason, validDate:pass.endDate });
      });
    }
    res.json(arr_passes);
  } catch (e) {
...
app.post("/verifyPass", async (req, res) => {
  ...
  try {
    ...
    if (check) {
      ...
      else if (check.endDate > currentTime && check.startDate <= currentTime) {
        await outPassCollection.deleteOne({
          rollno,
        });
        const data = {
...
app.post("/deletePass", async (req, res) => {
  try {
    ...
    console.log("two");
    const deletedOutPass = await outPassCollection.deleteOne({ rollno });
